...
class MdbClient extends EventEmitter {
  ...
  onMessageReceived(message) {
    ...
    console.log("mdbEvent", mdbEvent);
    this.emit("mdb/debug", mdbEvent);
    if (mdbEvent && mdbEvent.type === "statusVEND") {
      console.log("emitting statusVEND");
  ...
  cmd(method, methodResponse) {
    return new Promise((resolve, reject) => {
      ...
      const handleData = (data) => {
        console.log(`data in cmd: "${data}" waiting for "${methodResponse}"`);
  ...
  sendCommand(command) {
    console.log(`In mdbClient sending command: "${command}"`);
  ...
  async handshake() {
    ...
    console.log("disableResponse", disableResponse);
