...
class MdbClient extends EventEmitter {
  ...
  cmd(method, methodResponse) {
    return new Promise((resolve, reject) => {
      ...
      const handleData = (data) => {
        log.info("data in handle data", data);
        log.info("methodResponse", methodResponse);
  ...
  async startVendingCycle() {
    log.info("Starting vendig cycle");
  ...
  async completeVendingCycle(amount) {
    log.info("Completing vending cycle");
    ...
    log.info("Complet Vencing cycle response:", completeVendingCycleResponse);
    return completeVendingCycleResponse;
  }

  async cancelVendBeforePaymentRequest() {
    log.info("Stopping vending session before requesting credit");
    await this.cmd(
      MDB_SEND_METHODS.STOP_ONGOING_VEND_SESSION,
      MDB_RECEIVED_METHODS.VEND_CLIENT_ENABLE_RESPONSE,
    );
  }

  async denyCreditRequest() {
    log.info("Stopping vending session after requesting credit");
    // TODO: not sure what is the response for this method ???
    const cancelVendingCycleResponse = await this.cmd(
      MDB_SEND_METHODS.DENY_CREDIT_REQUEST,
      MDB_RECEIVED_METHODS.VEND_ACK,
    );
