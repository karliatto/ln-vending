...
class MdbClient extends EventEmitter {
  ...
  async enableCashlessPeripheral() {
    log.info("Calling enble cashless peripheral");
    const response = await this.cmd(
      MDB_SEND_METHODS.CLIENT_ENABLE,
      MDB_RECEIVED_METHODS.VEND_CLIENT_ENABLE_RESPONSE
    );
    console.log('response in enableCashlessPeripheral', response);
    return response;
  }

  async startVendCycle() {
    console.log("Calling start vending cycle");
    const response = await this.cmd(
      MDB_SEND_METHODS.CLIENT_START_VENDING_CYCLE,
      MDB_RECEIVED_METHODS.VEND_CLIENT_START_VENDING_CYCLE_RESPONSE
    );
    console.log('response in startVendCycle', response);
    return response;
  }
